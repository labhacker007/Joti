...
function AnalyticsDashboard() {
  ...
  const [settingsValues, setSettingsValues] = useState(() => {
    ...
    const saved = localStorage.getItem('orion-analytics-settings');
    if (saved) {
      ...
      if (parsed.cost && !parsed.roi?.cost_per_article) {
        ...
        delete parsed.cost;
        localStorage.setItem('orion-analytics-settings', JSON.stringify(parsed));
      }
    ...
    try {
      ...
      if (selectedReportType.id === 'mitre_attack_mapping') {
        ...
        try {
          ...
          brandingData = JSON.parse(localStorage.getItem('orion-company-branding') || '{}');
        }
...
<body>
    ...
    <footer class="footer">
        ...
        <div class="footer-text">${footer || `${companyName} • Threat Intelligence Report • Generated by Parshu`}</div>
    </footer>
</body>
</html>`;
  };
  ...
  const saveSettings = (type, values) => {
    ...
    localStorage.setItem('orion-analytics-settings', JSON.stringify(newSettings));
