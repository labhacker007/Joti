...
def _sanitize_for_subject(value: str) -> str:
    """Strip CRLF from email subject values to prevent header injection."""
    return _re.sub(r'[\r\n]+', ' ', str(value)).strip()


def _escape_html(value: str) -> str:
    """Escape HTML entities in user-supplied values for email bodies."""
...
class EmailNotifier:
    ...
    def send_hunt_alert(
        ...
        subject = f"Hunt Alert - {_sanitize_for_subject(hunt_platform.upper())} - {results_count} Results Found"
        ...
        <body style="font-family: Arial, sans-serif; padding: 20px;">
            ...
            <div style="padding: 20px; background: #f5f5f5; margin-top: 20px; border-radius: 10px;">
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    <tr>
                        ...
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;">{_escape_html(hunt_platform.upper())}</td>
                    </tr>
                    <tr>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;"><strong>Article:</strong></td>
                        <td style="padding: 10px; border-bottom: 1px solid #ddd;">{_escape_html(article_title)}</td>
                    </tr>
                    <tr>
            ...
            <div style="padding: 20px; margin-top: 20px;">
                <h3>Query Used:</h3>
                <pre style="background: #2d2d2d; color: #f8f8f2; padding: 15px; border-radius: 5px; overflow-x: auto;">{_escape_html(query[:500])}...</pre>
            </div>
            ''' if query else ''}
...
"""
    ...
    def send_report_share(
        ...
        subject = f"Threat Intelligence Report: {_sanitize_for_subject(report_title)}"
        ...
        <body style="font-family: Arial, sans-serif; padding: 20px;">
            <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%); color: white; padding: 20px; border-radius: 10px;">
                ...
                <p style="font-size: 14px; opacity: 0.8;">{_escape_html(report_type.title())} Report</p>
            </div>

            <div style="padding: 20px;">
                <h2>{_escape_html(report_title)}</h2>
                <p style="color: #666;">Generated by: {_escape_html(generated_by)} | Date: {datetime.utcnow().strftime('%Y-%m-%d')}</p>

                <div style="background: #f5f5f5; padding: 20px; border-radius: 10px; margin-top: 20px;">
                    {_escape_html(report_content).replace(chr(10), '<br>')}
                </div>
            </div>
