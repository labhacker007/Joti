...
export interface TimezoneContextValue {
  ...
  // Formatters
  formatDateTime: (dateString: string | Date | null | undefined, options?: FormatOptions) => string;
  formatDate: (dateString: string | Date | null | undefined, options?: FormatOptions) => string;
  formatTime: (dateString: string | Date | null | undefined, options?: FormatOptions) => string;
  getRelativeTime: (dateString: string | Date | null | undefined) => string;

  // Helpers
  getTimezoneLabel: () => string;
  getTimezoneAbbr: () => string;
  getTimezoneString: () => string;
  toUTC: (date: string | Date | null | undefined) => string | null;
...
export const TimezoneProvider = ({ children }: TimezoneProviderProps) => {
  ...
  const formatDateTime = useCallback((dateString: string | Date | null | undefined, options: FormatOptions = {}): string => {
    ...
    try {
      ...
      Object.keys(formatOptions).forEach(key => {
        if ((formatOptions as any)[key] === undefined) delete (formatOptions as any)[key];
      });
  ...
  const formatDate = useCallback((dateString: string | Date | null | undefined, options: FormatOptions = {}): string => {
    if (!dateString) return 'Unknown';
  ...
  const formatTime = useCallback((dateString: string | Date | null | undefined, options: FormatOptions = {}): string => {
    if (!dateString) return 'Unknown';
  ...
  const getRelativeTime = useCallback((dateString: string | Date | null | undefined): string => {
    if (!dateString) return 'Unknown';
  ...
  const toUTC = useCallback((date: string | Date | null | undefined): string | null => {
    if (!date) return null;
  ...
  const setUse24Hour = (val: boolean) => {
